...
function SidebarItem({ mod, variant, rank }: { mod: ModSummary; variant: SidebarVariant; rank?: number }) {
  ...
  return (
    <Link
      ...
      style={platformColor ? { '--tw-border-right-color': platformColor } as React.CSSProperties : undefined}
    >
      {rank !== undefined && platformColor && (
        <span
          ...
          style={{ color: rank === 1 ? platformColor : `${platformColor}73`, '--glow-color': platformColor } as React.CSSProperties}
        >
...
function SidebarList({ items, variant, showRank }: { items: ModSummary[]; variant: SidebarVariant; showRank?: boolean }) {
  return (
    <div className="divide-y divide-border/50">
      {items.map((mod, idx) => (
        <SidebarItem key={mod.id} mod={mod} variant={variant} rank={showRank ? idx + 1 : undefined} />
      ))}
    </div>
  )
}
