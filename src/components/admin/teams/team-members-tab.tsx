...
export function TeamMembersTab({ teamId, memberships, onMembersChange }: TeamMembersTabProps) {
  ...
  const onRemove = async (memberId: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/teams/${teamId}/members?memberId=${memberId}`, {
        method: 'DELETE',
      })
  ...
  const handleUnlink = async (member: TeamMember) => {
    ...
    try {
      ...
      onMembersChange(
        memberships.map((m) =>
          m.id === member.id ? ({ ...m, userId: null, user: null } as TeamMember) : m,
        ),
      )
    } catch (err) {
      toast({
                ...
                {isEditing ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      <Button size="sm" className="min-h-[44px]" onClick={() => onSaveEdit(m.id)}>
                        <Check className="ml-1 h-3 w-3" /> حفظ
                      </Button>
                    </div>
                  </div>
                ) : (
                    ...
                    <div className="flex items-center gap-3">
                      {getMemberAvatar(m as never) ? (
                        <Image
                          ...
                          width={36}
                          height={36}
                          src={getMemberAvatar(m as never)!}
                          alt=""
                      ...
                      <div>
                        <div className="flex items-center gap-2 flex-wrap">
                          {getMemberProfileUrl(m as never) ? (
                            <Link
                              href={getMemberProfileUrl(m as never)!}
                              target="_blank"
