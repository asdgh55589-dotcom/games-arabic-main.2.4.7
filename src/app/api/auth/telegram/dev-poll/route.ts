...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const upd of updates) {
      ...
      await sendMessage(botToken, user.id, `✅ تم تسجيل الدخول بنجاح!\n\nمرحباً ${displayName}، يمكنك الآن العودة إلى الموقع وستكون مسجّل الدخول تلقائياً.`)
      processed++
    }

    // تأكيد الاستلام لمسح الطابور
    if (lastUpdateId > 0) {
      await fetch(`https://api.telegram.org/bot${botToken}/getUpdates?offset=${lastUpdateId + 1}&timeout=0`).catch(() => {})
    }
