...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!rl.success) {
      return NextResponse.json({ error: AUTH_ERRORS.RATE_LIMITED, code: 'RATE_LIMITED' }, { status: 429 })
    }

    const body = await req.json().catch(() => null)
    if (!body || typeof body.id === 'undefined') {
      return NextResponse.json({ error: AUTH_ERRORS.TELEGRAM_FAILED, code: 'TELEGRAM_FAILED' }, { status: 400 })
    }
    ...
    if (!verifyTelegramAuth(data)) {
      return NextResponse.json({ error: AUTH_ERRORS.TELEGRAM_FAILED, code: 'TELEGRAM_FAILED' }, { status: 401 })
    }
    ...
    const displayName = [firstName, lastName].filter(Boolean).join(' ') || usernameRaw || `Telegram_${telegramId}`
    const email = `telegram_${telegramId}@telegram.local`
    // username must be unique — try telegram username, fallback to tg_ + id
    let username = usernameRaw ? usernameRaw.replace(/[^a-zA-Z0-9_]/g, '_').slice(0, 30) : `tg_${telegramId}`
    ...
    if (existingByUsername) {
      ...
      const viaOA = await db.oAuthAccount.findFirst({ where: { provider: 'telegram', providerAccountId: String(telegramId) } })
    ...
    if (oauth?.user) {
      ...
      const existingLink = await db.oAuthAccount.findFirst({ where: { provider: 'telegram', providerAccountId: String(telegramId) } })
      if (!existingLink) {
    ...
    if (ban.banned) {
      return NextResponse.json({ error: AUTH_ERRORS.USER_BANNED, code: 'USER_BANNED' }, { status: 403 })
    }
    ...
    const res = NextResponse.json({ success: true, redirectTo: '/', user: { id: user.id, username: user.username } })
    // Also set legacy ga_admin_role JWT for compatibility with proxy.ts / getSession fallback
    try {
      const fresh = await db.user.findUnique({ where: { id: user.id }, select: { tokenVersion: true, role: true } })
    ...
    // Set Better Auth cookie: better-auth.session_token
    ...
    return NextResponse.json({ error: AUTH_ERRORS.TELEGRAM_FAILED, code: 'TELEGRAM_FAILED' }, { status: 500 })
  }
}
