...
import { SPECIAL_ROLES, parseSpecialRoles, canHaveSpecialRole, type SpecialRole } from '@/lib/special-roles'
  ...
  const fetchUser = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setUser({ id: u.id, username: u.username, role: u.role, specialRoles: u.specialRoles || '' })
      }
    } catch {}
    setLoading(false)
  }
  ...
  const handleAdd = async (role: SpecialRole) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({ title: json?.error?.message || json?.error?.details || 'فشل', variant: 'destructive' })
      }
    } catch {
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center gap-3">
        <Link href={`/admin/users/${user.id}`} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-border hover:bg-accent">
          <ArrowRight className="h-5 w-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold">إدارة الأدوار الخاصة — {user.username}</h1>
          <p className="text-sm text-muted-foreground">الدور الأساسي: {user.role} — اختر الأدوار الخاصة المناسبة</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {(Object.keys(SPECIAL_ROLES) as SpecialRole[]).map((key) => {
          ...
          return (
            <Card key={key} className={`${has ? 'border-primary/30 bg-primary/5' : ''} ${!eligible ? 'opacity-60' : ''}`}>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <span className="text-xl">{config.icon}</span>
                  {config.nameAr}
                  {has && <Badge className="mr-auto bg-green-500 text-white"><Check className="h-3 w-3" /> مُضاف</Badge>}
                </CardTitle>
              ...
              <CardContent className="space-y-3">
                ...
                {!eligible && !has && (
                  <p className="text-xs text-destructive">غير مؤهل — يتطلب دور {config.minMainRole} أو أعلى</p>
                )}
                {has ? (
                  <Button variant="destructive" size="sm" onClick={() => setConfirmRemove(key)} disabled={!!actionLoading}>
                    {actionLoading === key ? <Loader2 className="h-4 w-4 animate-spin ml-1" /> : <X className="h-4 w-4 ml-1" />}
                    سحب الدور
                  </Button>
                ) : (
                  <Button size="sm" onClick={() => handleAdd(key)} disabled={!eligible || !!actionLoading}>
                    {actionLoading === key ? <Loader2 className="h-4 w-4 animate-spin ml-1" /> : <Shield className="h-4 w-4 ml-1" />}
                    منح الدور
                  </Button>
                )}
              </CardContent>
      ...
      <Dialog open={!!confirmRemove} onOpenChange={(open) => !open && setConfirmRemove(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>هل أنت متأكد من سحب دور {confirmRemove ? SPECIAL_ROLES[confirmRemove].nameAr : ''} من {user.username}؟</DialogDescription>
          </DialogHeader>
          <DialogFooter>
