...
export default function AddCreatorPage() {
  ...
  const handlePromote = async () => {
    ...
    try {
      ...
      toast({ title: 'تمت الترقية بنجاح', description: `${selectedUser.username} أصبح ${getRoleLabel(promoteRole)}` })
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/users', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: username.trim(), email: email.trim(), password, role: newRole }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data?.error || 'فشل الإنشاء')
      toast({ title: 'تم إنشاء المُعَرِّب بنجاح', description: `${username} بدور ${getRoleLabel(newRole)}` })
      router.push('/admin/creators')
      router.refresh()
    } catch (e) {
      toast({ title: 'خطأ', description: e instanceof Error ? e.message : 'فشل', variant: 'destructive' })
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6" dir="rtl">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">ترقية عضو موجود أو إنشاء حساب جديد بدور مُعَرِّب/ناشر</p>
      </div>

      <div className="flex gap-2">
        <Button variant={mode === 'promote' ? 'default' : 'outline'} className="flex-1 min-h-[44px]" onClick={() => setMode('promote')}>
          <UserPlus className="h-4 w-4 ml-2" /> ترقية عضو موجود
        </Button>
        <Button variant={mode === 'create' ? 'default' : 'outline'} className="flex-1 min-h-[44px]" onClick={() => setMode('create')}>
          ...
          <CardContent className="space-y-5">
            <div>
              ...
              <div className="relative mt-1">
                ...
                <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="اكتب اسم المستخدم..." className="pr-10" dir="ltr" />
              </div>
                  ...
                  {searchResults.map((u) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground truncate">{u.email} · {getRoleLabel(u.role)}</div>
                      </div>
                      {selectedUser?.id === u.id && <span className="text-xs font-bold text-primary">محدد ✓</span>}
                    </button>
                  ))}
                </div>
              )}
              {search.length >= 2 && !searchLoading && searchResults.length === 0 && (
                <p className="mt-2 text-xs text-muted-foreground">لا يوجد أعضاء بهذا الاسم (يُعرض الأعضاء فقط)</p>
              )}
            </div>
            ...
            {selectedUser && (
              <div className="rounded-lg border bg-card p-4">
                ...
                <div className="mt-4">
                  ...
                  <select value={promoteRole} onChange={(e) => setPromoteRole(e.target.value as never)} className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm">
                    <option value="creator">مُعَرِّب — ينشئ تعريباته الخاصة</option>
                    <option value="publisher">ناشر — ينشر من مصادر خارجية</option>
                  </select>
                </div>
                <Button className="mt-4 w-full min-h-[44px]" onClick={handlePromote} disabled={promoteLoading}>
          ...
          <CardContent>
            <form onSubmit={handleCreate} className="space-y-5">
              <div>
                ...
                <Input id="c-username" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="مثال: translator_ali" className="mt-1" dir="ltr" required />
              </div>
              <div>
                <Label htmlFor="c-email">البريد الإلكتروني *</Label>
                <Input id="c-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="ali@example.com" className="mt-1" dir="ltr" required />
              </div>
              <div>
                <Label htmlFor="c-password">كلمة المرور *</Label>
                <Input id="c-password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="6 أحرف على الأقل" className="mt-1" dir="ltr" required />
              </div>
              <div>
                <Label htmlFor="c-role">الدور *</Label>
                <select id="c-role" value={newRole} onChange={(e) => setNewRole(e.target.value as never)} className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm">
