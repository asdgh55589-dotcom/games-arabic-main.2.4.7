...
export default function AdminReportsPage() {
  ...
  const [assignable, setAssignable] = useState<
    Array<{ id: string; username: string; role: string; workload: number }>
  >([])
  ...
  return (
    <div className="space-y-8">
      <div className="relative overflow-hidden rounded-lg border border-border bg-card p-7">
        ...
        <div className="relative flex flex-col gap-8 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <h1 className="mt-5 text-4xl font-black text-white">مركز البلاغات والإشراف</h1>
          ...
          <div className="grid grid-cols-2 gap-4 lg:min-w-[420px]">
            <div className="rounded-2xl border border-red-500/15 bg-red-500/10 p-5">
              ...
              <div className="mt-3 text-4xl font-black text-red-300">{total}</div>
            </div>
      ...
      {reports.length === 0 ? (
        <EmptyState icon="inbox" title="لا توجد بلاغات" />
      ) : (
        <>
            ...
            <table className="w-full text-right">
              ...
              <tbody className="divide-y divide-border-light">
                {reports.map((report) => (
                  <tr
                    key={report.id}
                    className="text-sm transition-colors hover:bg-background-secondary"
                  >
                    <td className="px-5 py-4">
                      <span className="flex items-center gap-1.5">
                        {TARGET_ICONS[report.targetType]}
                        {REPORT_TARGET_TYPES[report.targetType as ReportTargetType]?.label ||
                          report.targetType}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-block rounded-full px-3 py-1 text-xs font-black shadow-lg ${REPORT_PRIORITIES[report.priority as ReportPriority]?.color || ''}`}
                      >
                        {REPORT_PRIORITIES[report.priority as ReportPriority]?.label ||
                          report.priority}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/admin/reports/${report.id}`}>
                        <Button
                          variant="ghost"
                          size="sm"
                          className="rounded-md border border-border bg-background-secondary text-muted-foreground hover:text-primary min-h-[44px]"
                        >
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div className="rounded-md bg-background-secondary p-2.5">
                      ...
                      <div className="mt-1 font-medium">
                        {report.assignedTo?.username || 'غير مُعيَّن'}
                      </div>
                    </div>
                  </div>
                  {report.description && (
