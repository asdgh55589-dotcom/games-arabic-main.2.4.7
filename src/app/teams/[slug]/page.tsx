...
export async function generateMetadata({ params }: TeamDetailPageProps): Promise<Metadata> {
  ...
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/api/teams/${encodeURIComponent(slug)}`, {
      next: { revalidate: 300 },
    })
    ...
    const description = team.description?.slice(0, 160) || `فريق ${team.name} للتعريب العربي - تعرف على أعمال الفريق وإنجازاته`
