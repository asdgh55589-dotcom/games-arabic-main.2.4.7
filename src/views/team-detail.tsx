...
export function TeamDetailPage() {
  ...
  const handleFollowToggle = async () => {
    if (!currentUser) {
      toast({ title: 'سجّل الدخول', description: 'يجب تسجيل الدخول لمتابعة الفرق', variant: 'destructive' })
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({ title: payload.isFollowing ? 'تمت المتابعة' : 'تم إلغاء المتابعة', description: team.name })
      } else {
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="grid place-items-center py-20 text-center">
          ...
          <Link href="/teams" className="mt-4 text-sm text-primary hover:underline">العودة لفرق التعريب</Link>
        </div>
      ) : (
        <>
          {/* البانر */}
          <div className="relative h-64 w-full overflow-hidden sm:h-72 lg:h-80">
            ...
            <div className="absolute right-0 top-0 left-0 z-10 p-4">
              <div className="flex items-center gap-2 text-sm text-white/70">
                <Link href="/teams" className="hover:text-white transition-colors">فرق التعريب</Link>
          ...
          <div className="mx-auto max-w-[1400px] px-4 lg:px-6" dir="rtl">
              ...
              <div className="flex flex-col sm:flex-row items-end gap-6">
                ...
                <div className="flex-1 pb-2 min-w-0 min-h-[120px] sm:min-h-[160px] lg:min-h-[200px] flex flex-col justify-end">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {team.isFeatured && (
                      <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-amber-500/30 bg-amber-500/15 text-amber-500" title="مميز">
                        <Star className="h-3 w-3 fill-amber-500" />
                      </span>
                    )}
                    {team.isOfficial && (
                      <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-primary/30 bg-primary/15 text-primary" title="رسمي">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!isOwner && (
                        ...
                        >
                          ...
                          {followersCount > 0 && <span className="text-slate-400">({formatNumber(followersCount)})</span>}
                        </button>
                      )}
                    </div>
                  </div>
            ...
            <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-7">
              ...
              <StatCell icon={UserPlus} label="عدد المتابعين" value={team.stats.followersCount} />
            ...
            <div className="mt-8 border-b border-slate-700">
              <div className="flex gap-0">
                {TEAM_TABS.filter((t) => {
                  const hidden = (team.hiddenTabs || '').split(',').map((s) => s.trim()).filter(Boolean)
                ...
                {(team.customTabs || []).filter((t) => t.visible).map((ct, i) => (
                  <TabButton
                    key={`custom-${i}`}
            ...
            <div className="py-6">
              ...
              {(team.customTabs || []).filter((t) => t.visible).map((ct, i) => {
                if (activeTab === `custom-${i}`) {
                  return (
                    <div key={`custom-content-${i}`} className="prose prose-invert max-w-none space-y-4 text-sm leading-relaxed text-slate-300">
  ...
  return (
    <div className="flex items-center justify-between gap-2 rounded-md border border-slate-700/80 bg-slate-800/60 px-8 py-1.5 transition-colors hover:border-slate-500">
      ...
      <span className="text-sm font-bold tabular-nums text-slate-100">
        {isText ? <span className="text-xs font-medium">{value}</span> : formatNumber(value as number)}
      </span>
    </div>
  )
}
              ...
              <tbody>
                {team.mods.map((m, i) => (
                  <tr key={m.id} className="border-b border-slate-700/50 transition-colors hover:bg-slate-800/30">
                    <td className="px-4 py-2.5 text-slate-500 font-mono text-xs">{i + 1}</td>
                    <td className="px-4 py-2.5">
                      <Link href={`/mod/${m.slug}`} className="font-medium text-slate-200 hover:text-primary transition-colors">
                        {m.name}
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-center font-mono text-xs tabular-nums text-slate-300">{formatNumber(m.downloads)}</td>
                    <td className="px-4 py-2.5 text-center font-mono text-xs tabular-nums text-slate-300">{formatNumber(m.endorsements)}</td>
                    <td className="px-4 py-2.5 text-center font-mono text-xs tabular-nums text-slate-300">{formatNumber(m.views)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t border-slate-600 bg-slate-800/50 font-bold">
                  ...
                  <td className="px-4 py-2.5 text-center font-mono text-xs tabular-nums text-slate-100">{formatNumber(team.stats.totalDownloads)}</td>
                  <td className="px-4 py-2.5 text-center font-mono text-xs tabular-nums text-slate-100">{formatNumber(team.stats.totalEndorsements)}</td>
                  <td className="px-4 py-2.5 text-center font-mono text-xs tabular-nums text-slate-100">{formatNumber(team.stats.totalViews)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
